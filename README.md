...
The function template `string signature_of(T)` gives you the signature of type T. Here T can be jni fundamental types such as jboolean, jlong (except jobject types), several c++ types such as (const) char*, string, vector, array, valarray, reference_wrapper, void, and function types whose return type and parameter types are of above types.

The result string of `string signature_of(T)` has static storage duration, so it's safe to use `signature_of(...).data()`.
